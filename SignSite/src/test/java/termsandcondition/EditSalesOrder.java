...
public class EditSalesOrder extends Initialstep
{
	String Sales1= "Acceptance of Purchase Order";
	String SalesLink= "http://13.210.33.250/www.loremipsum.com";
	String Sales2= "The Next Line";
	StringSelection Imagepath= new StringSelection("E:\\Nature.jpg");

	@Test
	public void editSalesOrder() throws InterruptedException, AWTException, IOException
	{
		...
		driver.findElement(By.xpath("(//p[text()= 'Sales Order Terms & Conditions']/..//button[@type= 'button'])[1]")).click();
		WebElement textarea= driver.findElement(By.xpath("//div[@class= 'ql-editor']"));
		textarea.click();
		textarea.sendKeys(Keys.CONTROL, "a");
		textarea.sendKeys(Sales1);
		Thread.sleep(2000);
		textarea.sendKeys(Keys.CONTROL, "a");
		driver.findElement(By.xpath("//button[@class= 'ql-link']")).click();
		Thread.sleep(1500);
		driver.findElement(By.xpath("//div[@class= 'ql-tooltip ql-editing']//input[@type= 'text']")).sendKeys(SalesLink);
		Thread.sleep(1500);
		...
		{
			testcase.log(PASS, "Sales Order Edited successfully and the '"+ confirmation + "' message is dispalyed");
			takescreenshot(driver, "Sales Order edited");
		}
		else
		{
			testcase.log(FAIL, "Sales Order did not edited. The '"+ confirmation + "' message is dispalyed");
			takescreenshot(driver, "Sales Order did not edited");
		}
	}
}
