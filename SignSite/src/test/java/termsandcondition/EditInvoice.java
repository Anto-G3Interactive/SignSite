...
public class EditInvoice extends Initialstep
{
	String Invoice1= "Acceptance of Purchase Order";
	String Invoice1Link= "http://13.210.33.250/www.loremipsum.com";
	String Invoice2= "The Next Line";
	StringSelection Imagepath= new StringSelection("E:\\Nature.jpg");
	
	@Test
	public void editInvoice() throws InterruptedException, AWTException, IOException
	{
		...
		driver.findElement(By.xpath("(//p[text()= 'Invoice Terms & Conditions']/..//button[@type= 'button'])[1]")).click();
		WebElement textarea= driver.findElement(By.xpath("//div[@class= 'ql-editor']"));
		textarea.click();
		textarea.sendKeys(Keys.CONTROL, "a");
		textarea.sendKeys(Invoice1);
		Thread.sleep(2000);
		textarea.sendKeys(Keys.CONTROL, "a");
		driver.findElement(By.xpath("//button[@class= 'ql-link']")).click();
		Thread.sleep(1500);
		driver.findElement(By.xpath("//div[@class= 'ql-tooltip ql-editing']//input[@type= 'text']")).sendKeys(Invoice1Link);
		Thread.sleep(1500);
		...
		{
			testcase.log(PASS, "Invoice Edited successfully and the '"+ confirmation + "' message is dispalyed");
			takescreenshot(driver, "Invoice edited");
		}
		else
		{
			testcase.log(FAIL, "Invoice did not edited. The '"+ confirmation + "' message is dispalyed");
			takescreenshot(driver, "Invoice did not edited");
		}
